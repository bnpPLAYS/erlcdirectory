...
const Navbar = () => {
  ...
  /** When true, nav row uses justify-between so center links sit closer to brand/actions (less dead space). */
  const [navTightSpacing, setNavTightSpacing] = useState(false);
  ...
  return (
    <>
      ...
      <header
        className={cn(
          'fixed left-0 right-0 z-50 flex justify-center pointer-events-none transition-[padding-left,padding-right] duration-300 ease-out',
          navTightSpacing ? 'px-2 sm:px-3' : 'px-3 sm:px-5',
        )}
        ...
        >
          ...
          >
            <img
              ...
              aria-hidden
            />
            <span className="text-sm font-semibold tracking-tight text-white truncate min-w-0">
              ERLC Directory
            </span>
          </Link>
