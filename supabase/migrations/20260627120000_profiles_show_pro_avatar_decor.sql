-- Opt-in Pro avatar orbit decoration (client-controlled when is_pro).

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS show_pro_avatar_decor boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.profiles.show_pro_avatar_decor IS
  'When true and is_pro, show animated orbit rings around the member avatar on profile, directory, and nav.';

-- Non-Pro members cannot keep this enabled (prevents arbitrary true without Pro).
