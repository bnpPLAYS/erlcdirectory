...
/**
 * Proxies Pro verification to Supabase Edge Function `verify-roblox-pro`.
 * Vercel only needs the same Supabase URL + anon key as the web app (no service role here).
 */
export default async function handler(request: Request): Promise<Response> {
  ...
  if (!supabaseUrl || !anonKey) {
    return new Response(
      JSON.stringify({
        ok: false,
        error: 'Server missing Supabase URL or anon key. Add VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY on Vercel.',
      }),
